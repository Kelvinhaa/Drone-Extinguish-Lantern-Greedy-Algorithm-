 ...
 * Implementation by Minh Hoang Ha.
...
void generate_lantern_paths(map_t* map){
    assert(map);
    // i, j here would represent for drones, lanterns respectively 
    // (same as task specifications for ease of understanding)
    int i, j, distance;
    int x_free, t_free;
    // Trackers for Tie-breaking
    int min_distance, early_free, smaller_index;
    int feasible_drone = FALSE;
    int free_x[MAX_DRONES], free_t[MAX_DRONES];

    for (i = 0; i < map->num_drones; i++) {
        free_x[i] = map->drone[i].x;
        free_t[i] = map->drone[i].trajectory_time;
    }

    // Iterate through lanterns and for each lanterns, find the feasible drone
    for (j = 0;j < map->num_lanterns;j++) {
        for (i = 0;i < map->num_drones;i++) {
            // Firstly, compute the free time of drone (t_free)
            // and position at that time (x_free)
            x_free = free_x[i];
            t_free = free_t[i];
            distance = abs(x_free - map->lantern[j].x);
            int arrival_times = map->lantern[j].arrival_time;
            
            if (distance > DRONE_SPEED*(arrival_times - t_free)) {
                continue;
            }
            if (!feasible_drone) {
            ...
            if (distance > min_distance) continue; // Criteria 1

            if (distance == min_distance) {
                if (t_free > early_free) continue;  // Criteria 2
                if (t_free == early_free && i > smaller_index) continue;  // Criteria 3
            }
            min_distance  = distance;
            early_free    = t_free;
            smaller_index = i;
        ...
        if (feasible_drone) {
            int to_x      = map->lantern[j].x;
            int to_time   = map->lantern[j].arrival_time;

            // Commit update for availability
            map->drone[smaller_index].lantern_path[map->drone[smaller_index].path_length++] = j;
            free_x[smaller_index] = to_x;
            free_t[smaller_index] = to_time;
        }
        feasible_drone = FALSE;
